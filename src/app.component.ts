...
type ActiveView = 'advice' | 'notes' | 'games' | 'music';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, AdviceComponent, NotesComponent, MusicComponent],
})
export class AppComponent implements OnInit {
  ...
  ngOnInit(): void {
    // FIX: Check if swUpdate is available before using it. This also narrows its type from `SwUpdate | null` to `SwUpdate`.
   ...
   * @param view - Идентификатор вида ('advice', 'notes', и т.д.)
   */
